  ...
  def jenaNode2RDFNode(r: JenaRDFNode): IO[RDFNode] = r match {
    ...
    case lit: JenaLiteral => {
      extendNS(lit.getDatatype.getURI) match {
        case Some(RDFNode.`RDFHTMLDatatypeIRI`) => // RH20220819
          ok(RDFHTMLLiteral(lit.getLexicalForm))
        case Some(RDFNode.`RDFXMLDatatypeIRI`) => // RH20220819
          ok(RDFXMLLiteral(lit.getLexicalForm))
