...
object Graph {
  ...
  /**
   * traverse the outgoing nodes of a node It currently only takes into account the ourgoing
   * arcs
   * @param node
   *   starting node
   * @param rdf
  ...
  def traverse(node: RDFNode, rdf: RDFReader): Stream[IO, RDFNode] = {
    ...
    def outgoingNotVisited(node: RDFNode, visited: List[RDFNode]): Stream[IO, RDFNode] =
      outgoing(node).filter(x => x != node && !visited.contains(x))

    // @annotation.tailrec
    // TODO: Refactor to be tailrec again
    def loop(stack: List[RDFNode], visited: List[RDFNode]): Stream[IO, RDFNode] = stack match {
