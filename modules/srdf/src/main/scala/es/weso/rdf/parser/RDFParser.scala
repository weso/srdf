 ...
 *
 * <p> The approach is similar to parser combinators but instead of sequence of characters, we
 * have RDF graphs available through an RDFReader
 */
trait RDFParser {

  /**
   * An RDFParser of values of type `a` takes a pointed node `RDFNode` and an `RDFReader` and
   * tries to obtain a value of type `a`
   */
  ...
  def checkString(obj: RDFNode, p: IRI): RDFParser[String] = obj match {
    ...
    case _ =>
      parseFail("Value of predicate " + p + " must be a string literal but it is: " + obj)
  }
  ...
  def checkDecimal(obj: RDFNode, p: IRI) = obj match {
    ...
    case _ =>
      parseFail("Value of predicate " + p + " must be a decimal literal but it is: " + obj)
  }
  ...
  /**
   * Returns a parser that obtains the type associated with the current node <p> Fails if there
   * are more than one type associated
   */
  ...
  /**
   * RDFParser that retrieves the object associated with current node for a given predicate <p>
  ...
  def checkTriples(ts: Vector[RDFTriple], n: RDFNode, p: IRI): RDFParser[RDFNode] =
    ts.size match {
      ...
      case _ =>
        parseFail(s"objectFromPredicate($p): More than one value from predicate $p on node $n")
    }

  /**
   * RDFParser that retrieves the set of iriObjects associated with the current node for a given
   * predicate <p>
   *
   * @param p
   *   predicate
  ...
  /**
   * A parser of the RDF List associated with the current node <p> Fails if there are more than
   * one iriObjects associated with `rdf_first` or `rdf_rest`
   */
  ...
  /**
   * Obtains the RDF list associated with a predicate for the current node If there is no value,
  ...
  def getIntegerFromValues(ts: Vector[RDFTriple], p: IRI, n: RDFNode) = ts.size match {
    ...
    case _ =>
      parseFail(
        s"integerLiteralFromPredicate($p): More than one value from predicate $p on node $n")
  }
          ...
          ls.fold(
            ...
            vs =>
              node match {
                ...
                case _ =>
                  Left(RDFException.fromString(s"node $node must be an integer literal"))
              })
  ...
  def someOf[A](ps: RDFParser[A]*): RDFParser[A] = {
    val zero: RDFParser[A] = fromEither(
      RDFException.fromString("someOf: none of the RDFParsers passed").asLeft[A])
        ...
        c.biflatMap(
          ...
          value =>
            parser.biflatMap(
              ...
              value2 =>
                parseFail(
                  s"oneOf: More than one parser passes. Two vaules that pass: $value and $value2")))
      /*{
        c.fold(
          _ => parser(n,rdf),
          _ => parser(n,rdf).
  ...
  def checkNodes[A](
      ...
      parser: RDFParser[A],
      n: RDFNode,
      pred: IRI): RDFParser[Option[A]] =
    ns.size match {
      case 0 => ok(None)
      case 1 => withNode(ns.head, parser).map(Some(_))
      case _ =>
        parseFail(s"opt fails because $n has more than one value for pred $pred. Values: $ns")
    }
